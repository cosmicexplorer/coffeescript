...
(function() {
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    literalToken() {
      var match, message, origin, prev, ref, ref1, ref2, ref3, ref4, ref5, skipToken, tag, value;
      if (match = OPERATOR.exec(this.chunk)) {
        [value] = match;
        if (CODE.test(value)) {
          this.tagParameters();
        }
      } else if (match = DISTINCT_DECL.exec(this.chunk)) {
        [value] = match;
      } else {
      ...
      switch (value) {
        ...
        case '[':
          ...
          this.ends.push({
            tag: INVERSES[value],
            origin: this.makeToken(tag, value)
          });
          break;
        case ')':
        case '}':
  ...
  DISTINCT_DECL = /^(?:@=)/; // distinct var decl which can have a specific comment attached.

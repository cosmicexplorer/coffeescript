...
(function() {
  var CoffeeScript, addHistory, addMultilineHandler, assert, fs, getCommandId, merge, nodeREPL, path, replDefaults, runInContext, sawSIGINT, transpile, updateSyntaxError, vm,
    indexOf = [].indexOf;
  ...
  addMultilineHandler = function(repl) {
    var finishMultilineBuffer, inputStream, multiline, nodeLineListener, origPrompt, outputStream, prepareMultilineInput, ref, resetOrigPrompt;
    ...
    finishMultilineBuffer = function() {
    ...
    resetOrigPrompt = function(cmd) {
      repl.setPrompt(origPrompt);
      return nodeLineListener(cmd);
    };
    prepareMultilineInput = function() {
      assert.ok(!multiline.enabled);
      multiline.enabled = !multiline.enabled;
    ...
    repl.on('line', function(cmd) {
      if (multiline.skipNextLine) {
        multiline.skipNextLine = false;
        return;
      }
      // console.error {cmd}
      if (multiline.enabled) {
        // console.error {cmd, s: 'here!!!'}
        if (indexOf.call(cmd, '') >= 0) {
